...
namespace {
  struct Mutation : public FunctionPass {
    ...
    explicit Mutation() : FunctionPass(ID) {}
...
char Mutation::ID = 0;
INITIALIZE_PASS_BEGIN(Mutation, "mutation", "Mutation Framework", false, false)
INITIALIZE_PASS_END(Mutation, "mutation", "Mutation Framework", false, false)

//static RegisterPass<Mutation> X("mutation", "Mutation for pThreads");

Pass *llvm::mutateCalls(){
  return new Mutation();
}
